...
A repo with neither backend **halts** — see `ref-tracker`'s detection step 6. Say the repo has no tracker, offer `bd init`, and file nothing. Never write the items to a markdown list instead: a list nobody maintains is where follow-ups go to be forgotten, and it puts a file that must survive inside a tree that gets deleted.
...
Invoke `ref-tracker` and run its detection step:
