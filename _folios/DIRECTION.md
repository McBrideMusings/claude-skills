...
face every page gets by not choosing, and on an identity-first folio that reads as no decision made.

The scale is already built: `--f-3xl` down to `--f-sm` on a 17px root, and `--f-3xl` is the display
step — one per folio, on the title, never on a section heading. A title only a third larger than
...
grey at 3.2:1 and the accent at 2.96:1 — so every link on every folio failed AA — and nobody saw it
until the folio's own `c` check was run in both themes. Light grey "for elegance" on a near-white
