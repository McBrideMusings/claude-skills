...
```bash
bash ~/.claude/skills/_artifacts/specimens/build.sh
```
...
- **Real content only.** Every specimen is about the artifact store itself, so the copy is
...
- **The built HTML is not tracked.** `build.sh` writes to `tmp/claude/specimens/`, which is
  gitignored and age-pruned. The fragments are the artifact.
