...
Don't write new top-level planning / phase / feature docs in `docs/` — file an issue on the repo's tracker instead (invoke `ref-tracker`). `roadmap.md` is the only forward-looking doc.
```
