...
Read by `gui` critique/audit mode (and the `review`/`verify` engines when the domain is `gui`).
...
`../../gui/layers/` cell rather than patching the symptom. (This audit judges *fidelity to the decisions
...
- **Surface fix vs deeper-layer issue.** The key judgement: is this a copy/layout fix, or a symptom whose
  ...
  never settled the term. Route deeper issues to `../../gui/layers/conceptual-model.md` or
  `interaction-flow.md` (via `gui`), not to a surface patch.
...
- **Object consistency.** No shapeshifters (same object in significantly different forms across
  ...
  OOUX failure modes from `gui`; at the surface they're visible symptoms.)
...
When `gui` runs a post-code critique/audit, run this fidelity pass **alongside** the `design.md`
craft lenses and the `slop.md` catalog. Tag each finding: **surface-fix** (fix here) or **deeper-layer**
(route to the named `../../gui/layers/` cell). A finding is still a verdict-plus-reason — the reason here
