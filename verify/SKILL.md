...
- **Never commit a project verify skill — gitignore it the moment you create it.** It is one person's rig for driving one program on one machine: it carries local paths (`/Volumes/…`, a simulator UDID, a checkout location), tokens and endpoints used to exercise the thing, and verification choices that are the operator's, not the project's. On a public repo that's a leak. So immediately after writing the file, append its path relative to the repo root — e.g. `.claude/skills/verify/` or `apps/ios/.claude/skills/verify/` — to the repo-root `.gitignore` (skip if already listed), commit only that `.gitignore` line, and never `git add` the skill itself. Being *in* the project is what makes the recipe correct; being *out of git* is what makes it safe. If you find one already tracked, say so and stop — untracking it needs a history rewrite the user has to approve.
