...
**Never write an absolute path into a `run` line — a repo-relative path is always correct, from any subdirectory.** The tool `chdir`s to the directory holding `admin.toml` before running anything, so `run = "bun test"` or `run = "./scripts/dev.ts"` already resolve there regardless of where `admin` was invoked from. An absolute path pointing at a checkout is never necessary and is actively dangerous: `admin worktree` *copies* `admin.toml` into each new worktree rather than symlinking it, so an absolute path baked into a `run` line keeps pointing at the checkout it was written in — usually the primary — even from inside a fresh worktree that shares no files with it. The failure is quiet: `admin dev`/`test`/`vet`/`db` all succeed, ports open, the database migrates, and nothing in the output says which checkout actually ran. (Tracked upstream as `admin-1xgi`: `admin check` should reject this outright; until then, write it correctly by hand.) A path that legitimately lives outside any checkout — `/opt/homebrew/bin/psql`, `/usr/bin/env`, an installed binary on `PATH` — is fine; the rule is about a `run` line naming a path inside a project's own tree.
