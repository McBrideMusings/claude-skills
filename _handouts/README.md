# `_handouts/` — shared visual-handout store
...
CRITIQUE.md       <- the one batched pass between building an handout and handing it over
kinds/
...
harness/
  chrome.css      <- the one palette every piece of harness furniture consumes, plus the
                     insets that describe how much of the window the handout actually gets
...
Generic chrome any handout can carry, declared in one table (`WIDGETS`) in the tool.
Adding one is a row there plus its files here — not three new branches in the build path.
`handout kinds` prints the current set; `--with NAME` adds one to a kind that does not
...
nothing at all until their key is pressed, so an handout you hand to someone else looks
...
the model asks for it per handout.
...
The tool that assembles all of it: `~/.claude/tools/handout`.
...
blob-URL workers all run. Only `fetch()` is blocked (`origin 'null'`), and a hermetic handout has
nothing to fetch — data goes in as a JS literal. **This is why no web server is part of the design.**
`handout serve` exists as a contingency and shells out to the already-installed `python3`.
...
an handout to a hosted page (see `~/.claude/CLAUDE.md` §5); the file itself is the portable thing.
