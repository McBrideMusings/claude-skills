...
Resolve the issue backend once — invoke `ref-tracker` and run its detection step — before resolving any selector — the commands below are given per backend.
...
| `epic:<id>` or `epic:<name>` | `bd list --parent <epic-id> --status open --json`; resolve a name through `bd list -t epic --json` first. Add `--parent` to `bd ready` for the unblocked subset. | Sub-issues of that issue: the GraphQL `subIssues` field on `Issue` (see `../ref-tracker/github.md`). |
