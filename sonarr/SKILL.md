---
name: sonarr
description: "Query Sonarr to discover TV series and episodes with file paths and metadata. Formats results as etv-station channel TOML items."
user_invocable: true
---
...
### Step 3 — Format as TOML

Shape each episode result into `[[rule.items]]` blocks using the field mapping table. Skip episodes with no file (`episodeFileId` is 0 or absent).

**For large result sets (10+ items): hand the formatting to a Haiku sub-agent.** Pass it the digested results from `ctx_execute` and the field-mapping table from this skill, and have it return the TOML blocks. Keeps the long render output off the parent's context and runs cheaper.
...
## Field mapping: Sonarr → etv-station TOML

| Sonarr API field | TOML field | Notes |
|---|---|---|
