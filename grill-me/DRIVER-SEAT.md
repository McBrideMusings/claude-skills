...
**Shape B — user-written text** (for example, the plan `grill-me` Grill mode is interviewing): make no edit. Emit the changes as a numbered slate in chat — one row per change, stating what moves, where (section), and why, in the plan's own vocabulary — closing with: *"Type `go` to apply my picks as described, or answer per row (`1 fix, 3 skip, rest file`)."* Apply only the accepted rows, and only once the reply arrives.
