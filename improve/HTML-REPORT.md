...
- **Lens** — one line naming the rule, craft lens, domain cell, or lateral technique that produced the finding, as an openable path plus the section (`ARCHITECTURE.md § deletion test`, `_domains/gui/opportunities.md § motion`, `lateral/techniques/worst-idea.md`). The aspect eyebrow says which agent ran; this says which sentence fired. Never just the aspect name.
