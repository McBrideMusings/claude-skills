...
- **Lens** — one line naming the rule, craft lens, domain cell, or lateral technique that produced the finding, as an openable path plus the section (`ARCHITECTURE.md § deletion test`, `_domains/gui/opportunities.md § motion`, `lateral/techniques/worst-idea.md`). The aspect eyebrow says which agent ran; this says which sentence fired. Never just the aspect name. **This is a citation, not a route** — the reader clicks it to check the claim, so it stays a path to the exact file and section even where the skill reaches that knowledge by invoking `ref-architecture` or `lateral <technique>`. Do not rewrite these into skill invocations.
