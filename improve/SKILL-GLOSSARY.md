...
The domain model for what makes a skill great. A skill exists to wrangle determinism out of a stochastic system; the root virtue is **Predictability**, and every term below is a lever on it. This is the disclosed reference for [`WRITING-SKILLS.md`](WRITING-SKILLS.md).
...
- **Tier 2 — external roll.** The pick is taste or resonance, so justification can't help — the model justifies its argmax fluently, and prompting harder only moves the argmax. The choice leaves the model: a grounded shortlist, an index from outside (`secrets.randbelow`), a re-roll. The worked mechanism is `ref-gui/direction.md`.
