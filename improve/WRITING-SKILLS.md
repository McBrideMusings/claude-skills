...
**Granularity** is how finely you divide skills, and the first two cuts each spend one of the two loads, so split only when the cut earns it. Four cuts:

- **By invocation** — split off a **model-invoked** skill when you have a distinct **leading word** that should trigger it on its own, or another skill must reach it. You pay **context load** for the new always-loaded **description**, so that independent reach has to be worth it.
- **By sequence** — split a run of **steps** when the steps still ahead (a step's **post-completion steps**) tempt the agent to rush the one in front of it (**premature completion**). Keeping them out of view encourages the agent to do more **legwork** on the current task.
- **By axis** — the **axis split**. Keep ONE skill and split its _knowledge_, not the skill, into per-context files an engine reads at run time. The root **SKILL.md** stays a verb (`generate`, and the engines reading the merged domain store): it holds the how-to once and, per run, loads exactly the one axis file the context selects. The files live in an `_axis/` directory whose leading `_` and absent `SKILL.md` keep it from registering as its own skill. **Sibling or child is decided by readers, not by convention:** an axis read by more than one skill is a top-level sibling (`_domains/` feeds a dozen skills; `_generate/` is read by `generate` and by `_domains/gui/direction.md`), and an axis only its own engine reads belongs inside that skill, since a subdirectory never registers either. Putting a single-reader axis at the top level clutters the skills root and implies a sharing that does not exist. Reach for it when a skill does the same _process_ across many contexts (asset types, target platforms, knowledge domains) and only the reference differs per context — one engine, N thin axis files, each a `README.md`-documented set the engine dispatches over. Spends neither load: no new **description** (still one skill) and nothing for the human to remember (still one name). The cost it pays is indirection — the how-to and the per-context knowledge sit in different files — so it earns its place only when the contexts are numerous or growing enough that inlining them all would **sprawl** the root. Adding a context is then dropping one file into `_axis/`, which is also how a mined skill gets **folded in** (see `repo-analysis`).
- **By knowledge** — the **`ref-` cut**. Split *knowledge* out of the skills that use it into bare documents, and give the subject one thin **`ref-<subject>`** skill whose body is a **file map and nothing else**: one row per document saying when to open it. Workflow skills own gates and processes; `ref-` skills own maps; the documents own the knowledge. Reach for it when the same body of knowledge is restated in two or more skills, or when a document has no reliable route to it. Rules, all load-bearing:
  - **The body is only the map.** No procedure, no summary of what the documents say. A body big enough to act on without opening anything gets acted on without opening anything — read rate falls as the pointing document grows (measured 0–70%, see [POINTERS.md](POINTERS.md)). **Thinness is the forcing function, not a token saving.**
  - **The `description` is capped at 25 words** and carries triggers only. There is no procedure to describe, so there is nothing else it can legitimately hold. That is ~38 tokens of permanent **context load** per subject, against ~3,500 for the current model-invoked set.
  - **Model-invoked, always.** A `ref-` skill exists to be reachable when the task calls for it; `disable-model-invocation: true` would strip the only route it has.
  - **One store, never a fork.** The documents stay wherever they already live and keep their existing readers. A `ref-` skill adds a route to knowledge; it never becomes a second copy of it.
...
| --- | --- | --- |
| creates, reads, closes, comments on, or labels a tracked item | [`../ref-tracker/_detect.md`](../ref-tracker/_detect.md) then the resolved backend's verb table | `gh issue …`, `bd …` |
