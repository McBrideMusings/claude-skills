...
- **⛔ A pull resets labels, `issue_type` and `priority`.** Pull only to collect a UI edit,
  scoped `--issues <id>`.
- **`area:`/`human` never travel outward.** Apply them with `gh issue edit`.
- **Working an issue means slices plus a verify and a land bead under it**, pushed like any
  other. Shape: [breakdown.md](breakdown.md).
