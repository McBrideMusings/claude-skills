...
The Land bead exists for the same reason: it is a slate row, taken with `go`, never inside a pass or a brief. The one run that carried "push and open the PR" into the worker did exactly that, with four unanswered product questions pasted into the PR body, and the only reason the questions existed was that nobody had been asked. A pass cannot open a PR because it runs no `git push`; the orchestrator does not either until the row is accepted. **This gate covers the outward move only** — `git push`, a PR, a merge into the default branch — never the merge back into the feature branch, per [`SKILL.md`](SKILL.md) § The unit is a slice.

**You merge each pass as it returns**, from the primary checkout, after re-verifying against a base that may have moved. A linked worktree shares the object store, so every commit is already visible there — no fetch needed. **Do not offer this as a row and do not wait for `go`** — a swarm that verifies five passes and then asks permission to merge each one has stalled the run for nothing, and the second batch behind it never dispatches. Merge, stop the agent, stop its surfaces, remove the worktree, then continue.
