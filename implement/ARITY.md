...
Resolve the selector into a frozen queue first — issue numbers, `#range`, `label:X`, `milestone:X`, `epic:X`, `followups`, `papercuts`. Then work it one at a time: pass, verify loop, `wrap-up continuous <worktree>`, next. Re-resolving the selector between items gets a different queue, because the backlog moved while you were landing branches.
...
Every pass gets its own worktree and they run at once. The human is involved at exactly two moments: the scope gate before anything is dispatched, and the closing report at the end of the run, with a gate for each item it halted on ([`SKILL.md`](SKILL.md) § The gate). In between there is no channel from a pass back to a person, and no way for one to exist. An item that turns out to be ambiguous costs its whole dispatch and waits for a `backlog shape` pass.
...
**Starting a swarm is the standing authorization to land each item as it clears.** Once a round is dispatched, this session calls `wrap-up continuous <worktree>` on each pass as its verify loop clears — no row, no `go`, no per-item ask. This covers the outward move too — a merge to the default branch, or a PR on a collaborative repo — the same way starting a queue or swarm already authorizes a PR under `../wrap-up/SKILL.md` Step C. The one run that carried "push and open the PR" into the worker itself did exactly that, with four unanswered product questions pasted into the PR body, and the only reason the questions existed was that nobody had been asked — which is why the authorization sits at the swarm's own dispatch, in `wrap-up`, never inside a pass or a brief.

`wrap-up continuous <worktree>` does the rest: re-verifies against a base that may have moved, commits any straggling review/simplify fixes, lands the branch (merge or PR), copies the verdict into the repo's own `$(~/.claude/tools/repo-slug --path <repo>)/verify/<item>.json`, removes the worktree and branch, and closes the tracked item — the teardown mechanics are in `../wrap-up/SKILL.md` Step C. Stop the pass's agent and its surfaces first, then call it, then continue to the next return; under `continuous` this runs with no relay in between, since relay only fires once, at the end of the whole run.
...
**A selector matching more than 8 items pilots first.** Dispatch a batch of at most 5, merge those, and check the cost and failure rate in `/tasks` before dispatching the rest. Sequential arity already caps a whole run at 20 items; swarm arity has no run-level cap, so an oversized selector dispatches its entire count at once unless this step catches it first. **The pilot releases itself** — a clean pilot dispatches the remainder without a second ask, since the user already accepted the whole selector when the round was gated. Report the pilot's outcome in the same message that says the rest are going out; do not stop and wait on it.
