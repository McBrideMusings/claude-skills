...
**A brief that names a start command names its stop.** Whenever a brief tells the pass to launch a surface — a dev server, a static file server, a simulator, an emulator — it also says to stop it before returning, and assigns that pass its own port or device so siblings in a swarm do not collide on one. A pass that starts a server and exits leaves it running against a worktree the orchestrator is about to remove, and in a swarm that is one orphan per agent. The orchestrator stops whatever it started itself, per [`SKILL.md`](SKILL.md)'s verify loop; this line covers the half that runs inside the pass, where the orchestrator cannot see it.
