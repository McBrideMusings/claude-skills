...
2. **Explicit argument.** If the arguments contain a bare issue number (e.g. `/implement 1118`) or an issue reference (a beads ID like `myproj-zb8`), that issue **is** the work item. Resolve the backend by invoking `ref-tracker` and confirm it exists — `bd show <id> --json` on beads, `gh issue view <n> --json number,title,state` on GitHub; if it's closed or missing, halt and say so. Skip triage entirely — the user named the target.
