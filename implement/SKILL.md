...
**A session already standing in the item's own worktree runs `implement <issue>` inline, unasked.** The test: this checkout is a linked worktree (`git rev-parse --git-dir` differs from `git rev-parse --git-common-dir`) and its branch name contains the item id, lowercased — `bead/admin-vnnw` for `admin-vnnw`. The session that dispatched this one re-checks and lands the branch, so this session works the item itself, launches the blind `code-reviewer` once the build is green, and ends at a commit on this branch. A queue, a swarm, or a feature worktree whose branch names no single item still dispatches passes.
