...
**You are REQUIRED to capture a real screenshot of the running app and look at it. There is no soft path around this and a flaky tool is not an excuse.** Build and launch the project's own debug/dev build, `screencapture` its window, and drive it into each state you need (open the modal, add the 2nd tab, switch the theme). If the screenshot tool times out, retry / relaunch / kill-and-respawn it; if an instance is in the way, use the debug build; if it's a native app, screencapture the window; use the project's run/verify skill or documented dev-drive path. "It timed out / another instance was running / it's not a web page / it was busy" are NOT blocks — they are work you must do.

**Capture hygiene — a screenshot timeout is almost always self-inflicted, NOT a broken tool. Rule out your own interference BEFORE you even think the word "blocked."** The failure mode that has actually happened: the agent runs pre-capture DOM surgery to "help" the shot — freezing animations, `visibility:hidden`, and above all `canvas.getContext(...).loseContext()` on a live WebGL/xterm canvas — then `page.screenshot()` hangs forever, because `loseContext()` destroys the GPU context the compositor needs to produce the frame. It looks *exactly* like a tool timeout, and the agent then declares an "exhausted, proven tool limitation" that is neither. Two hard rules:
- **Capture FIRST, on the untouched page.** Take the screenshot before any freeze/hide/dispose. Manipulate only *after* you have the frame, if at all. **Never `loseContext()`, dispose, or tear down a GPU/canvas/renderer on the page you are about to capture.** (A blinking cursor or a live WebGL loop does *not* prevent capture — proven.)
- **Isolate before you cry wolf.** If a capture times out, screenshot a trivial `data:text/html` page in the same tool. If *that* captures instantly, the tool is fine and the fault is your page or your manipulation — go fix that. Only if a plain page ALSO fails to capture may you treat the tool itself as the problem. Skipping this isolation and jumping to "the tool is broken" is a false BLOCK — and a false BLOCK is a worse failure than a slow one, because it abandons the gate while sounding rigorous.

**Only after genuinely exhausting a real capability may you STOP — and then it is a loud BLOCK, never a claim of done.** "Genuinely exhausting" means you have already done the capture-hygiene checks above (captured first, no context-destroying manipulation, isolated on a trivial page) and the tool still fails on a page it should trivially handle. If you truly cannot render and compare (a proven, named limitation), the run halts and the FIRST line of your response is: "⛔ BLOCKED — I have NOT visually verified this. The sync is unfinished," followed by exactly which step failed and what you need. You are FORBIDDEN from claiming it matches, AND from the limp "want me to spell out a checklist / verify when you run it?" offer — that offer is the tell you skipped the gate and is banned. `tsc`/tests/clean-build are never a substitute (a build passes while the UI is visibly wrong — that is the exact bug this gate catches).

**The ONLY time the visual pass moves off you is an explicit, documented prohibition** (a CLAUDE.md rule or memory that literally bars the agent from opening/screenshotting the app — not mere inconvenience). Even then you do NOT declare done: (a) produce the **numeric** proof yourself (computed-style / DOM / box-metric extraction that returns real values, never a screenshot), and (b) open with the loud "⛔ NOT visually verified by me" block and hand the **user** a per-surface checklist to sign off surface-by-surface. That is a flagged-incomplete handoff, never a "matched."
