...
Parallel to `ref-game-dev`'s build arc (which orchestrates `_domains/game/`). It does **not** own code *correctness*, tests, or verification — those are the engines (`review`, `tdd`, `verify`, `diagnose`), which gain UI competence by reading `_domains/gui/` and `_domains/{web,apple}/` when the `gui` label is in scope. The split: design judges whether the thing is *well designed*; the engines judge whether the code is *correct*. Don't reimplement the engines here.
