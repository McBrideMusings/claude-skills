...
Read by the `diagnose` engine when the domain is `game` — loaded on top of the platform axis, not instead of it.

Adapted from majidmanzarpour/threejs-game-skills.

What to watch when a game misbehaves — the bug classes a game's shape produces. Platform-agnostic:
this says *what to instrument and what pattern to look for*; the platform axis (`_platforms/<p>/
...
numbers with the platform profiler via `_platforms/<p>/profiling.md`.
