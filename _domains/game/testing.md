...
Read by the `testing` engine when the domain is `game` — loaded on top of the platform axis, not instead of it.
...
progresses, the player responds, nothing softlocks, no errors. The platform axis (`_platforms/<p>/
testing.md`) supplies the harness/driver; this axis says what to assert.
...
score, softlock, errors), never performance. Real perf numbers come from `_platforms/<p>/profiling.md`.
