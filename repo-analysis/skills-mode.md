...
- **Fold as axis** — their skill is one context of a process an engine of yours already runs (a platform, a domain, an asset type). Drop it into the matching `_axis/` directory as a new file, not a new skill. This is the **axis split**; mechanics live in [`../improve/WRITING-SKILLS.md`](../improve/WRITING-SKILLS.md). (`generate`'s per-type files were folded in from `majidmanzarpour/threejs-game-skills` this way.)
