...
Write to `/private/tmp/claude/<repo-slug>/repo-analysis-<ref-name>.md`, where `<ref-name>` is a short slug naming the reference repo(s) — e.g. `repo-analysis-cmux.md`, or `repo-analysis-hlsjs-shaka.md` for multiple references. Never write to a bare `repo-analysis.md`: each analysis gets its own file, so a later run against a different reference never overwrites an earlier report. If the exact target filename already exists (a re-run against the same reference), overwrite it. `/private/tmp/claude/<repo-slug>` is the directory `~/.claude/tools/repo-slug --path` prints (it creates the directory). The `## Findings` section below follows [`../CHAT-FORMAT.md`](../CHAT-FORMAT.md) §Finding (additive — the Ours/Theirs/Suggested-fix fields extend it, not replace it). ALWAYS use this exact structure:
...
1. **Map findings against existing tracking first — codebase over questions.** Before asking anything, check the repo's existing plans: open issues on whichever backend the repo uses (`bd list --status open --json` / `gh issue list`, backend resolved by invoking `issues`) and any plan files. The tracker is the whole forward-looking picture — there is no roadmap doc to cross-check. Bucket every actionable finding as either **already planned** (an existing issue/plan covers it — candidate for an augmentation comment citing the reference's pattern) or **unplanned** (candidate for a new issue). Show the user this mapping as context before the first question.
...
3. **End with a confirmed slate.** Summarize the final list — new issues with one-line scopes, augmentation comments per existing issue — per [`../CHAT-FORMAT.md`](../CHAT-FORMAT.md) §Slate row, before creating anything. Issue bodies match the repo's existing issue conventions and cite the reference's file:line pattern sources; augmentation comments name the concrete code both sides. Type `go` to apply my picks as described, or answer per row (`1 fix, 3 skip, rest file`).
