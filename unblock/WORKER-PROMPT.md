# Unblock worker — conflict resolution, CI diagnosis, feedback, reporting

This is the source text for `agents/unblock-worker.md`'s system prompt (in the `~/.claude`
repo, a separate item — not this repo). It is the whole diagnosis-and-repair procedure the
`unblock` skill's entry now only summarizes.

Four things block a branch, and this owns all four:
