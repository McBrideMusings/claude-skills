...
**Not here — these are already handled globally** in `~/.config/git/ignore`, and repeating them per-project is the duplication problem: macOS noise (`.DS_Store`, `._*`, `.Spotlight-V100`), `admin.toml` and `/admin`, `.claude/settings.local.json`. `.claude/domain` is the opposite case — commit it; see [`_detect.md`](../_detect.md).
