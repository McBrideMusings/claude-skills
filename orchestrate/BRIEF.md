...
>
> **Commit and push your branch BEFORE any optional quality pass** — before a review agent, a simplification pass, a docs tidy, a second look at anything. Those are worth doing and they go after, on top of a pushed commit; anything they change gets its own commit and push. The reason is not tidiness: an uncommitted worktree is the only state nobody but you can land or recover, and the optional passes are exactly where workers die.
>
> **Write your verdict file only after that push, and put the real branch head in its `commit` field** — `git -C <worktree-path> rev-parse HEAD`. A verdict written before the commit names a commit that does not contain the work, which the orchestrator reads as a `PASS` for code that is not there.
>
...
| commit and push before any optional quality pass | the ordering is the fix; every other clause here is a guard. Observed: a worker finished the work, wrote a `PASS` verdict, spawned a simplification agent, ended its turn waiting for it, and left five files uncommitted with **zero** commits on the branch. Had it pushed first, the identical stranding would have cost a simplification pass instead of the whole slice. This shrinks the window rather than policing the symptom |
| verdict written after the push, `commit` = real branch head | that same worker's verdict named the base commit its branch was cut from, so it read as a clean `PASS` for a branch containing nothing. A verdict is evidence only if it points at the code it describes — and the orchestrator's staleness check is phrased around a verdict being *behind* the head, which this case slips straight through |
