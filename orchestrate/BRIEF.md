...
| `item` = the tracker id, in the field and the filename | a stranded worker wrote `/private/tmp/claude/<repo-slug>/verify/undefined.json` with the issue *title* in `item`, so the id was in neither place. Step 5 read it as a worker that stopped without a verdict, and step 7's exact-path check would have deleted a complete `FAIL` verdict with the worktree |
