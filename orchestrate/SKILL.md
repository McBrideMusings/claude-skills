...
They were not equivalent options that got trimmed for tidiness. A worker dispatched as a single `Agent` call — which is what both the subagent and herdr transports did — runs all six implement phases in **one context that grows all pass**. Measured across session logs: single-context workers ran far more turns than a split pass, reached far deeper context, and together accounted for a disproportionate share of all token spend.
...
5. **Worktree still there?** — the last thing before the transport is touched, for **every** worktree in the batch, not just the one being dispatched:
   ...
   **Why this check exists (`term-bvlt`).** Creation succeeding is not evidence the worktree is there when the worker starts: a worktree at the tip of the default branch has no commits of its own and a clean tree, which is exactly what `~/.claude/tools/git-sweep.sh` treats as "merged into main" and removes — even from this same session's own `Stop` hook, mid-round. `git-sweep.sh` now refuses a worktree carrying a `SWARM-WORKER` or `SELF-LAND` marker, so this check is what makes the *next* such cause loud at dispatch instead of silent until the round returns.
...
- **Arm a long heartbeat (1200–1800s)** as a backstop, whatever the transport's own signal is. It catches a dead Monitor, a notification that never fires, and a worker stuck in a state that never changes.
  ...
  **The mechanism, because naming only the outcome does not work.** `ScheduleWakeup` is the only tool that schedules a wake, and it exists **only inside a `/loop`** — a session started as a bare `/orchestrate` cannot call it. So arming the heartbeat is a step, not an assumption: **immediately after the first `dispatch(…)` returns, invoke the `loop` skill in dynamic mode** — `Skill(loop, "check the running orchestrate round: sweep every live worker's worktree, land anything finished, dispatch the next round when the frontier frees")` — with no interval, so you pace it yourself. From that point `ScheduleWakeup` is callable, and every turn ends with one:
  ...
  A wording that only names the outcome ("add a long heartbeat") gives an orchestrator no route to it outside a loop, and the only compliant move left is to dispatch with no backstop and say so — which is why arming it is written here as the concrete step above, not the result alone.
...
**And the reverse also holds: a notification is not a death.** On the subagent transport one fires whenever an agent stops with no live background children, so an agent that left a build running notifies, then resumes when the build ends. Judge by whether the worktree is changing — new commits, a verdict file, touched sources — not by the notification. `ListAgents` showing a "completed" worker `running` again is the tell that a recovery dispatch would collide with it.
...
**Run the `cat-file` first, and stop there when it fails.** A sha that names no object in the repo was never read out of git, so the verdict measured nothing — a different, worse verdict class than a stale one, not a harsher label on the same one, and a naive comparison misreads it as merely stale. `verify` runs before anything is committed, so the only sha it can honestly record is the parent of the commit its work becomes; Wrap then makes exactly one commit. On an honest pass those match, and when they don't, something was committed after the verdict was written and is shipping unverified.

**Do not expect a `commit` field, and never ask anyone to add one.** Earlier versions had `verify` write `commit` (meaning the parent) and a later `restamp` stage rewrite it to the wrap sha — refused by the safety classifier as audit tampering, correctly, since it asked an agent to write "this commit was verified" about a commit no stage had verified. The field is named for what it truthfully holds so nothing has to correct it afterwards.

**A verdict whose `verified_parent` is the base commit is a different failure — emptiness, not staleness** — the worker wrote its verdict before committing anything. `git -C <worktree> log --oneline <default-branch>..<branch>` returning empty is the check to run, because it holds even when the verdict omits the sha entirely, and a comparison phrased only in terms of the head's parent lets this straight through.
...
1. **Scope check** — `git -C <worktree> diff --stat <default-branch>...<branch>` and compare the touched paths against the issue's stated scope. This is what actually enforces "format only what you touched" and "no repo-wide formatters" ([Worker rules the brief must carry](#worker-rules-the-brief-must-carry)) — a brief clause with nothing checking it at land time is a rule nobody enforces. A diff confined to the issue's area, plus incidental touches (an index file, a lockfile) is normal; a diff touching files well outside that scope — a reformat of unrelated files, an edit to a module the issue never named — **halts the merge for a look**, not a silent proceed. Land after confirming the extra scope is legitimate, or send it back for correction.
2. Fetch, then compare `git merge-base origin/main <branch>` against `origin/main`'s head.
3. **Unchanged** → the worker's verdict still describes this exact tree; merge and push.
4. **Moved** → another branch landed since this worker forked. Merge, then **re-run `verify` on the merged result** before pushing. Two branches can each pass alone and break together.
5. **Conflict** → abort the merge, leave the worktree and branch intact, escalate. Never force-resolve.
...
**List the directory; do not just `test -f` the path you expect.** A `test -f` against one exact name passes vacuously when the worker wrote a differently-named file, and `--force` then deletes the only copy — including a complete `FAIL` verdict naming the exact cause of the failure, found only by listing the directory rather than by looking for the expected name. That kind of loss survives only when the tree also happens to be dirty, which forbids teardown for an unrelated reason. **Any `.json` in there that is not `<item>.json` blocks teardown**: copy it out under a name that includes the worker's issue and branch, then decide. Two workers in one round can both write a same-named stray file, and once copied to the primary checkout they are indistinguishable — so never copy one out under the name it already has.

**A live process in the worktree forbids teardown exactly as uncommitted work does.** `worktree remove --force` deletes the directory out from under whatever is standing in it, and that process keeps running against a path that no longer exists — a bundler or dev server left running inside the worktree keeps serving from a path that is now gone, and the failure surfaces later, inside whatever was consuming it, reading as a broken build rather than as teardown. A clean, fully-merged tree passes every other check and gives no warning that this is about to happen.

**Refuse, do not name-and-remove.** A swarm retires workers unattended, so "removed, and this killed the process" is still an unattended removal — the sentence lands in a round report nobody reads until the app is already broken. Deferring costs one worktree's disk; the branch is already merged and the round moves on. Say which condition fired and which process holds it — worktree, issue, pid, command — so the next pass retires it rather than re-deriving why it was skipped.
...
**A gitignored file the worker created is invisible to every check above, and `hooks/worktree-remove-locals-guard.sh` denies the removal when one exists.** `git status --short` reads git's view, so a file git is told to ignore leaves it empty — a worker-created `admin.toml`, gitignored globally and never committed, passes every teardown check above and then `--force` takes the repo's only copy of its build, test and dev commands. The guard compares the worktree against the primary checkout on the `pattern` names in `~/.config/repo/config.toml`, the same list `repo populate` brings in, and names the file it found. Copy that file to the primary checkout, then re-run the removal.
...
A stopped worker can still surface a report afterwards — a late notification, a resumed turn, an agent that had a background child outlive it — potentially several more times, over hours, after its branch has already been merged, its issue closed, and its worktree removed, describing work in a directory that no longer exists.
...
**Symptom you got it wrong:** more live workers than issues you can name a reason for. Reconcile every wake, against the transport's own listing — `herdr tab list --workspace "$HERDR_WORKSPACE_ID"` or the running-agent list. A worker with no reason to exist is a slot the frontier is waiting on — a pane left open with nothing left to do, waiting on a decision that will never come, while ready issues sit with nowhere to go.
...
Give every item in **Next batch** and **Follow-up needed** its own pick first, so `go` has something to mean. Missing this closing line is a near-universal failure across long runs' final turns — the one where re-typing the whole slate costs most. `hooks/go-hatch-check.py` catches it on `Stop`; this line is what stops it happening.
...
- **Workers never land, and never push — and step 3's `SWARM-WORKER` marker is what makes that true.** Every clause below is prose an agent has already been observed reading and ignoring; the marker turns them into a denied tool call. A worker's Wrap stage is a four-step commit, not `wrap-up` with its landing and tracker steps subtracted — `implement.js` generates it from `land: 'caller'`. It ends at the commit: a linked worktree shares the primary checkout's object store, so the orchestrator already has every worker commit and lands from there.
  ...
  **Do not reinstate "do not push to the default branch" as the rule.** That was the rule, in the brief and in `implement.js`, and workers merged into `main` and pushed to `origin/main` anyway. `git checkout main` does fail in a worktree — the sentence claiming that makes landing impossible was wrong, and it stopped everyone looking. The workers ran `git -C <primary-checkout>`, which bypasses that protection entirely. A worker whose pass contains no push at all has nothing to route around.
- **Workers never invoke `/code-review`.** `implement.js` has a Review stage that is handed the worktree path and diffs against the sha the pass started from with `git -C`. Nested inside a worker, `/code-review` resolves its own cwd to *this* session's primary checkout and can review a clean `main` instead — "no changes to review" reads as "no findings", so the gate passes on a diff nobody saw. The returned object carries `review: {findings, blocking}`. A worker that reached Review and could not read a diff **halts** and returns `halted_on: "review"` instead of a result — there is no completed pass whose diff went unread, so nothing to check for.
