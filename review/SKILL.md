...
**Low-severity, non-blocking finding → offer fix-then-approve, not just comment.** When the survivors are all `low`-severity, non-blocking quality notes (a lone `best-practice`/`contracts`/`architecture` nit and nothing broken), do **not** present "Comment / Approve as-is / Neither" as the only choices — that buries a fixable nit or ships it unaddressed. Lead with **fix it on the branch, then Approve**: the finding is small and the merge isn't gated, so the cleanest outcome is landing the one-line fix and approving clean. Offer it as the first option whenever the fix is small and safe; fall back to plain Comment only when the fix genuinely needs the author's intent (e.g. it would change behavior, or hinges on something you can't verify without them) — then say *why* it's hand-to-author rather than fix-then-approve.
