...
- **Describe the architecture before slicing, repo mode only — there is no diff to extract intent from.** One or two paragraphs, written by reading the tree's actual structure (not the README): what the major pieces are, how they're wired together, what pattern the repo actually follows. Then compare that paragraph against the README's own description of the architecture (if one exists) and hand **the contradiction, not the paragraph**, to the `docs-drift` lens as its brief for this check — see `axes/docs-drift.md`. **Do not forward the paragraph itself to any other Phase 04 lens.** The same anchoring risk that keeps the Phase 03c intent table away from the `slop` lens in the table below (a lens handed "the intent is X" rationalizes dead structure as serving X) applies here at the whole-repo level — a lens told what the architecture is *supposed to be* will read every file through that lens instead of judging the structure on its own terms.

- **Slice first. One pass over a whole codebase is not the default and never was tractable.** Before Phase 04, partition the scope into **coherent subsystem slices** — by feature area or bounded context, following the repo's own structure (top-level packages/apps, then domain folders), not by file count or alphabet. Name each slice for what it *is* (`league-domain`, `gems-store`, `migrations-infra`).

  **Order the printed slice list by churn against size, not partition order.** For each slice: `git log --stat --since="6 months ago" -- <slice-path>` for churn (count files touched × commits, or just commits touching the slice — keep the heuristic simple, this is an ordering, not a scoring model) and total line count across the slice's files for size. Sort slices with both high churn and large size first; a large file nobody touches or a small file touched constantly is usually fine, so neither alone floats a slice up. Print the ordered list before Phase 04 starts. **This changes the order only — every slice still gets the full Phase 04→06 lens set; there is no sampling and nothing is skipped.** It will sometimes be wrong (a large, frequently-touched slice can be a feature under active construction, not debt) — that's an acceptable cost for an ordering and would not be for a filter.

  Run the full Phase 04→06 lens set **once per slice, in the printed order**. Findings from every slice merge into one report.
...
- Issue references in the commit messages (`#123`, `Closes #45`, `Fixes #67`, or a beads ID like `myproj-zb8`) — fetch via `bd show <id> --json` or `gh issue view <N>`, with the backend resolved by invoking `issues`.
...
- If nothing found, briefly ask the user where the spec is. If they say "no spec", the Spec sub-agent will skip and report "no spec available". In unattended contexts (sweep mode, the `workflow` transport) or if the question goes unanswered, skip asking and proceed as "no spec found".
...
**Transport fork.** Phases 04–06c run inside a workflow script by default on every route, or here in the session when the `session` token was given or the `Workflow` tool cannot be called from where this pass runs — see [TRANSPORT-WORKFLOW.md](TRANSPORT-WORKFLOW.md). *Which lenses run, what each brief contains, and every forwarded directive below are identical either way*; only where the agents execute differs. Everything from Phase 07 onward is unaffected.
...
**Bites is where the concrete values live**, and it does two jobs with one line. Left of the arrow is the exact input Phase 05b feeds to the running code — `A♠2♦3♣4♥5♠ at showdown`, `entries=[]`, `a row written before the 2026-03 migration`. Right of the arrow is who it hurts in real units and when it fires — pots, dollars, seconds, rows, players, requests. `Users may be affected` is not a Bites line, and neither is `edge case → possible data loss`. On `negative-space`, `architecture`, `test-debt`, `dependency-debt` and `docs-drift` the left side is omitted and the line is impact-only.
